...
class DateHelerTests: XCTestCase {
    ...
    func testRelativeDateStringFromTimeStamp() {
        ...
        components.month = -6
        components.day = -4
        let oneYearSixMonthsAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        let oneYearSixMonthsAgoStr = DateHelper.relativeDateStringFromTimestamp((oneYearSixMonthsAgo?.timeIntervalSince1970)! * 1000)
        
        // 10 mo
        components.year = 0
        components.month = -10
        let tenMonthsAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        let tenMonthsAgoStr = DateHelper.relativeDateStringFromTimestamp((tenMonthsAgo?.timeIntervalSince1970)! * 1000)
        
        // 7 mo 20 days
        components.month = -7
        components.day = -19
        let sevenMonthsTwentyDaysAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        ...
        components.month = 0
        components.day = -11
        let elevenDaysAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        let elevenDaysAgoStr = DateHelper.relativeDateStringFromTimestamp((elevenDaysAgo?.timeIntervalSince1970)! * 1000)
        
        // 8 days 4 hours
        components.day = -8
        components.hour = -4
        let eightDaysFourHoursAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        ...
        let tenHoursAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        let tenHoursAgoStr = DateHelper.relativeDateStringFromTimestamp((tenHoursAgo?.timeIntervalSince1970)! * 1000)
        
        // six hours 15 minutes
        components.hour = -6
        let sixHoursFifteenMinutesAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        ...
        let thirtySevenMinutesAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        let thirtySevenMinutesAgoStr = DateHelper.relativeDateStringFromTimestamp((thirtySevenMinutesAgo?.timeIntervalSince1970)! * 1000)
        
        // 4 min 56 sec
        components.minute = -4
        let fourMinutesFiftySixSecondsAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        ...
        components.minute = 0
        let fiftySixSecondsAgo = calendar.dateByAddingComponents(components, toDate: rightnow, options: [])
        ...
        let nummatches = monthsAndDays.numberOfMatchesInString(sevenMonthsTwentyDaysAgoStr!, options: [], range: range)
